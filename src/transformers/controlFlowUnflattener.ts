...
function processVarDeclarations(statements: any[], hoistedVars: Set<string>) {
            ...
            exit(path: any) {
                ...
                for (let i = path.key - 1; i >= 0; i--) {
                    ...
                    if (siblingPath.isVariableDeclaration()) {
                        const declarator = siblingPath.node.declarations.find((d: any) => t.isNodesEquivalent(d.id, stateVar) && d.init && t.isNumericLiteral(d.init));
                        if (declarator) {
                ...
                for (const switchCase of switchStatement.cases) {
                    ...
                    const caseBody = switchCase.consequent.filter((stmt: any) => !t.isBreakStatement(stmt));
                ...
                function unflatten(currentState: number, recursionStack: Set<number>): any {
                ...
                if (success) {
                    ...
                    if (initializerPath.isVariableDeclaration()) {
                        ...
                        const declIndex = declarators.findIndex((d: any) => t.isNodesEquivalent(d.id, stateVar));
                        if (declIndex !== -1) {
