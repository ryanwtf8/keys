...
function valueToNode(value: any) {
    if (value === undefined) {
...
export const inlineArrayBuilder = {
    visitor: {
        Function(path: any) {
            ...
            bodyPath.traverse({
                AssignmentExpression(assignmentPath: any) {
                    ...
                    if (evalResult.success) {
                        ...
                        const statement = assignmentPath.findParent((p: any) => p.isStatement());
                        if (statement) {
            ...
            if (hasChanges) {
                for (const tracker of trackers.values()) {
                    const sortedPaths = Array.from(tracker.pathsToRemove).sort((a: any, b: any) => b.key - a.key);
                    for (const p of sortedPaths) {
                        if (p && !(p as any).removed) {
                            (p as any).remove();
                        }
                    }
                }
